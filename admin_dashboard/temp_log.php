    ...
    <?php 
      ...
      *******************************************************/
       ...
       //echo "$s1";
        ...
        $sql = "SELECT `temperature`, `time` FROM `logs1` WHERE `time` > '$s1' && `temperature` >= '$warning' ORDER BY `time` ASC" ; //the query
        $number = 1;
        $result = mysqli_query($conn, $sql);//execute query
        $append = "";
      ...
      ****************************************/
         $sql = "SELECT `temperature` FROM `logs1` WHERE `time` > '$s1'"; //query for ploting graph
         $result = mysqli_query($conn, $sql); //execute query
         if (mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
              echo "<script> arraygraph.push(".$row['temperature'].");</script>"; 
