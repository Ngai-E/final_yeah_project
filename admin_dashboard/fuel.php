...
<html lang="en">
    ...
    <?php 
      ...
      *******************************************************/
       ...
       //echo "$s1";
        ...
        $sql = "SELECT `fuel_level`, `time` FROM `logs1` WHERE `time` > '$s1' && `fuel_level` >= '$warning' ORDER BY `time` ASC" ; //the query
        $number = 1;
        $result = mysqli_query($conn, $sql);//execute query
        $append = "";
      ...
      ****************************************/
         $sql = "SELECT `fuel_level` FROM `logs1` WHERE `time` > '$s1'"; //query for ploting graph
         $result = mysqli_query($conn, $sql); //execute query
         if (mysqli_num_rows($result) > 0) {
            echo "<script> var arraygraph = [];</script>";   //used to plot graph
            echo "<script> var labelgraph = [];</script>";   //used to plot graph
            // store the values of fuel_level in an array
            while($row = mysqli_fetch_assoc($result)) {
              echo "<script> arraygraph.push(".$row['fuel_level'].");</script>"; 
                          ...
                          <div class="content-panel" >
                            <h4><i class="fa fa-angle-right"></i> Table of threshold values</h4><hr><table class="table table-striped table-advance table-hover">
                              ...
                              <tr>
                                  ...
                                  <td><span class="hidden-phone">levels below this should indicate no fuel left</span></td>
                                  <td>
                          ...
                          <div class="content-panel">
                            <h4><i class="fa fa-angle-right"></i>&#32;Graphical representation of fuel level in the past week</h4>
                              <div class="panel-body text-center">
                                  <canvas id="fuel_graph" height="300" width="400"></canvas>
                              </div>
                          </div>
  ...
  <script> 
    ...
    new Chart(document.getElementById("fuel_graph").getContext("2d")).Line(lineChartData);
